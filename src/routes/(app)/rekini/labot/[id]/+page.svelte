<script lang="ts">
	...
	const initialClientId = invoice.clientId.toString();
	...
	let items = $state(
		invoice.items.length > 0
			? invoice.items.map((i: any) => ({
					...
					isAutoFilled: products.some(
						(p: any) =>
							p.title === i.description ||
							p.translations?.some((t: any) => t.title === i.description)
					)
				}))
			: [
					{
	...
	function handleDescriptionInput(e: Event, index: number) {
		...
		const match = findProductByAnyTitle(val);
		if (match) {
	...
	$effect(() => {
		...
		if (selectedClientDetails && selectedClientId !== initialClientId) {
	...
	$effect(() => {
		...
		const updated = snapshot.map((item) => {
			...
			if (match) return { ...item, description: getProductTitle(match, lang) };
			return item;
		});
	...
	// Reactive invoice number (can be regenerated)
	let currentInvoiceNumber = $state(invoice.invoiceNumber);
	let regenerating = $state(false);

	async function handleRegenerateNumber() {
		regenerating = true;
		try {
			const res = await fetch(`/api/invoices/${invoice.id}/regenerate-number`, { method: 'POST' });
			const data = await res.json();
			if (!res.ok) {
				toast.error(data.error || 'Failed to regenerate invoice number');
				return;
			}
			currentInvoiceNumber = data.invoiceNumber;
			toast.success(`Invoice number updated to ${data.invoiceNumber}`);
		} catch (e) {
			console.error(e);
			toast.error('An error occurred while regenerating the invoice number');
		} finally {
			regenerating = false;
		}
	}
								...
								<tr>
									...
									<td class="border border-black px-0 py-0" dir="rtl">
										<input
											type="date"
											name="issueDate"
											class="relative w-full border-none bg-transparent px-4 py-0.5 pl-6 text-end text-sm focus:ring-0 [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:left-0"
								...
								<tr>
									...
									<td class="border border-black px-2 py-0.5 text-right">
										<div class="flex items-center justify-end gap-1">
											<span class="font-bold text-gray-700 italic">{currentInvoiceNumber}</span>
											{#if $isAdmin}
												<button
													type="button"
													title="Regenerate invoice number"
													class="ml-1 rounded p-0.5 text-gray-400 hover:text-gray-700 disabled:opacity-40"
