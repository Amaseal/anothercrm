<script lang="ts">
	...
	import type { InvoiceWithRelations } from '../../+layout.svelte'; // Reuse type or just use simplistic one logic
	import X from '@lucide/svelte/icons/x';
	import * as m from '$lib/paraglide/messages';
	import FormError from '$lib/components/form-error.svelte';

	// We can interpret the loaded item as generic object for display
	let { data, form } = $props();
</script>
		...
		<Card.Root class="custom-scroll relative max-h-[90vh] w-full max-w-md gap-2 overflow-y-auto">
			<Card.Header>
				...
				<h2 class="text-lg font-semibold">
					Delete Invoice {data.item?.invoiceNumber}?
				</h2>
			</Card.Header>
