<script lang="ts">
	...
	function handleDescriptionInput(e: Event, index: number) {
		...
		if (match) {
			...
			// items[index].unit = 'gab.'; // Assuming default, or add unit to product schema later
		}
	}
	...
	<div class="max-h-[90vh] w-full max-w-[70vw] overflow-scroll rounded-lg">
		<div class="min-h-screen bg-gray-100 p-8">
			<div class="mx-auto mb-4 flex items-center justify-between print:hidden">
				<h2 class="text-xl font-bold text-gray-700">{m['invoices.add_invoice']()}</h2>
				<a href="/rekini" class="text-sm text-gray-500 hover:underline"
					>{m['components.delete_modal.cancel']()}</a
				>
			</div>
				...
				<form method="POST" use:enhance>
					...
					<div class="mb-6 flex flex-col items-end text-sm font-bold">
						...
						<div
							class="flex w-64 justify-between border-r border-b border-l border-black bg-white px-2"
						>
							<span>{m['invoices.summary.vat']()} {taxRate}%</span>
					...
					<div class="mt-12 flex justify-end gap-4 border-t pt-6 print:hidden">
						...
						>
						<Button type="submit" size="lg" class="px-8">{m['components.save']}</Button>
					</div>
				</form>
			</div>
		</div>
