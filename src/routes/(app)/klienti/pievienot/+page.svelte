				...
				<form method="POST" use:enhance>
					<div class="flex items-center justify-stretch gap-2">
						<div class="flex-1">
							...
							<Input placeholder={m['clients.name_placeholder']()} required name="name" />
						</div>
						<div class="flex-1">
					...
					<div class="flex items-center gap-2">
						...
						<div class="flex-1">
							...
							<Input placeholder={m['clients.vat_number_placeholder']()} name="vat_number" />
						</div>
					</div>

					<div class="flex-1">
						<Label>{m['clients.address']()}</Label>
						<Input placeholder={m['clients.address_placeholder']()} name="address" />
					</div>
					<div class="flex items-center gap-2">
						<div class="flex-1">
							<Label>{m['clients.bank']()}</Label>
							<Input placeholder={m['clients.bank_placeholder']()} name="bank" />
						</div>
						<div class="flex-1">
							<Label>{m['clients.swift']()}</Label>
							<Input placeholder={m['clients.swift_placeholder']()} name="swift" />
						</div>
					</div>

					<Label>{m['clients.bank_account']()}</Label>
					<Input placeholder={m['clients.bank_account_placeholder']()} name="bank_account" />

					<Label>{m['clients.phone']()}</Label>
					<Input placeholder={m['clients.phone_placeholder']()} name="phone" />
