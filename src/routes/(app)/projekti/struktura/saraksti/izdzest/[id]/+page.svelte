<script lang="ts">
	...
	import type { TabTranslation, TabGroup } from '$lib/server/db/schema.js';
	...
	const getTranslation = (translations: TabTranslation[]) => {
...
<svelte:head>
	<title
		>{m['components.delete_modal.title']({
			item: getTranslation(data.item?.translations as TabTranslation[])?.name as string
		})}</title
	>
</svelte:head>
