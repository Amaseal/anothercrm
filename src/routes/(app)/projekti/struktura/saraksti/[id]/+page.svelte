<script lang="ts">
	...
	let selected: string | undefined = $state(data.tab?.groupId?.toString() || undefined);
</script>
					...
					<form method="POST" use:enhance>
						{#each locales as locale}
							...
							<Input
								...
								name="title-{locale}"
								required
								value={data.tab?.translations.find((t) => t.language === locale)?.name || ''}
							/>
						{/each}
						<Label for="color">{m['tabs.color']()}</Label>
						<Input type="color" name="color" id="color" value={data.tab?.color || '#000000'} />
						...
						<div class="nav">
							<Button type="submit" variant="outline"><Plus /> {m['components.save']()}</Button>
						</div>
					</form>
				</div>
			</Card.Content>
